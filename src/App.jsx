...
const pages = ['home', 'about', 'testimonials', 'skills', 'contact']
...
const pageVariants = {
  enter: (w) => ({ x: w.x * 200, y: w.y * 200, opacity: 0, scale: 0.88, rotate: w.x * 12 }),
  center: { x: 0, y: 0, opacity: 1, scale: 1, rotate: 0 },
  exit: (w) => ({ x: w.x * -220, y: w.y * -220, opacity: 0, scale: 1.08, rotate: w.x * -15 }),
}
...
function Equalizer() {
  ...
  return (
    <div className="pointer-events-none fixed bottom-0 left-0 right-0 z-[1] flex h-48 items-end gap-[1.5px] opacity-45 md:h-56 md:gap-[2px]">
...
function Nav({ current, onChange }) {
  ...
  return (
    ...
    >
      <button onClick={() => onChange(0)} className="font-display text-xl tracking-wide text-black cursor-pointer md:text-3xl">
        Z
      ...
      <nav className="flex flex-wrap gap-1 text-xs font-black uppercase tracking-wide md:gap-2 md:text-sm">
        {labels.map((label, i) => (
          <button
            key={label}
            onClick={() => onChange(i)}
            className={`border-2 px-2 py-1.5 transition cursor-pointer md:px-3 md:py-2 ${
              current === i
                ? 'border-black bg-[#00e5a0]'
                : 'border-transparent hover:border-black hover:bg-[#00e5a0]'
            }`}
          >
            {label}
          </button>
  ...
  return (
    <div className="fixed bottom-6 left-1/2 z-50 flex -translate-x-1/2 gap-3">
      {Array.from({ length: total }).map((_, i) => (
        <button
          key={i}
          className={`h-3 w-3 border-2 border-black transition-all duration-500 cursor-pointer ${
            current === i ? 'bg-[#00d4ff] scale-125' : 'bg-[#e8f6ff]'
          }`}
        />
      ))}
    </div>
...
function NavButton({ dir, onClick }) {
  return (
    <motion.button
      ...
      whileTap={{ scale: 0.95 }}
      className="fixed z-50 flex h-10 w-10 items-center justify-center border-4 border-black bg-[#e8f6ff] text-base font-black shadow-[4px_4px_0_#050505] transition hover:bg-[#00e5a0] cursor-pointer md:h-12 md:w-12 md:text-xl"
      style={dir === 'prev' ? { left: 16, top: '50%', translateY: '-50%' } : { right: 16, top: '50%', translateY: '-50%' }}
    >
      {dir === 'prev' ? '◀' : '▶'}
    </motion.button>
  )
...
function Hero({ onChange }) {
  return (
    <motion.div variants={container} initial="hidden" animate="visible"       className="mx-auto grid w-[calc(100%-24px)] max-w-6xl items-center gap-4 py-2 my-auto max-h-full overflow-y-auto md:py-8 lg:grid-cols-[1.15fr_0.85fr] lg:gap-10 lg:py-12">
      <div>
        ...
        <motion.div variants={container} className="mt-3 flex flex-wrap gap-4 md:mt-5">
          <motion.button variants={scaleIn} onClick={() => onChange?.(2)} className="comic-btn bg-[#00d4ff] text-[#050505] cursor-pointer">
            Testimonial
          </motion.button>
          <motion.button variants={scaleIn} onClick={() => onChange?.(4)} className="comic-btn bg-[#00d4ff] text-[#050505] cursor-pointer">
            Hire Me
          </motion.button>
        </motion.div>
      </div>
      ...
      >
        ...
        <div className="flex items-center justify-center py-2 md:py-4">
          <motion.div
            className="relative h-32 w-32 overflow-hidden rounded-full border-[6px] border-black bg-[#00d4ff] shadow-[8px_8px_0_#050505] md:h-72 md:w-72 md:shadow-[14px_14px_0_#050505]"
...
function About() {
  return (
    <motion.div variants={container} initial="hidden" animate="visible" className="mx-auto flex w-[calc(100%-24px)] max-w-6xl flex-col py-2 my-auto max-h-full overflow-y-auto md:py-12">
      <SectionTitle eyebrow="Origin Story" title="About Me" />

      <div className="grid gap-4 lg:grid-cols-[1.4fr_0.8fr_0.8fr] lg:gap-6">
        <motion.article
           variants={{ hidden: { opacity: 0, x: -40 }, visible: { opacity: 1, x: 0, transition: { duration: 0.35, ease: easeIn } } }}
          className="comic-panel bg-[#e8f6ff] p-4 md:p-7 lg:col-span-1"
        >
...
function Testimonials() {
  ...
  return (
    <motion.div
      ...
      initial="hidden"
      animate="visible"
      className="mx-auto flex h-full w-[calc(100%-24px)] max-w-6xl flex-col items-center justify-center gap-3 py-4 md:gap-5 md:py-8"
    >
      ...
      <motion.div
        variants={scaleIn}
        className="relative flex w-full max-w-[min(92vw,620px)] flex-1 flex-col overflow-hidden border-[5px] border-black bg-[#e8f6ff] shadow-[10px_10px_0_#050505] min-h-0"
      >
...
function Contact() {
  return (
    <motion.div
      ...
      initial="hidden"
      animate="visible"
      className="mx-auto flex w-[calc(100%-24px)] max-w-6xl flex-col py-2 my-auto max-h-full overflow-y-auto md:py-12"
    >
      <div className="relative overflow-visible border-[6px] border-black bg-[#e8f6ff] p-4 text-center shadow-[8px_8px_0_#050505] md:p-12 md:shadow-[14px_14px_0_#050505] comic-contact">
...
function RotateWarning() {
  ...
  return (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      className="fixed inset-0 z-[200] flex flex-col items-center justify-center bg-[#0d1b2a]"
    >
      <div className="mx-4 max-w-sm border-[6px] border-[#00d4ff] bg-[#e8f6ff] px-8 py-10 text-center shadow-[14px_14px_0_#050505]">
        <svg viewBox="0 0 64 64" fill="none" className="mx-auto h-24 w-24">
          <rect x="18" y="4" width="28" height="56" rx="4" stroke="#00d4ff" strokeWidth="3" />
          <line x1="28" y1="52" x2="36" y2="52" stroke="#00d4ff" strokeWidth="3" strokeLinecap="round" />
          <path d="M52 22a18 18 0 0 1-40 0" stroke="#00d4ff" strokeWidth="3" strokeLinecap="round" strokeDasharray="4 6" />
          <path d="M12 22a18 18 0 0 1 40 0" stroke="#00d4ff" strokeWidth="3" strokeLinecap="round" />
          <polyline points="44,16 52,16 52,24" stroke="#00d4ff" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" />
          <polyline points="20,28 12,28 12,20" stroke="#00d4ff" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" />
        </svg>
        <h2 className="mt-6 font-display text-2xl uppercase tracking-wide text-black md:text-3xl">
          Putar Perangkat
        </h2>
        <p className="mt-4 text-sm font-bold leading-relaxed text-black md:text-base">
          Gunakan mode landscape atau putar device Anda untuk pengalaman terbaik.
        </p>
      </div>
    </motion.div>
  )
...
const pageComponents = [Hero, About, Testimonials, Skills, Contact]
...
export default function App() {
  ...
  const [warp, setWarp] = useState({ x: 1, y: 0 })
  ...
  return (
    <>
      ...
      <motion.main
        ...
        className="relative flex h-screen w-screen flex-col overflow-hidden bg-[#d4ecf7] text-black"
      >
      <div className="pointer-events-none fixed inset-0 z-0">
        <div className="absolute inset-0 halftone-bg opacity-20" />
        <div className="absolute inset-0 bg-[linear-gradient(135deg,rgba(255,49,49,0.18),transparent_35%),linear-gradient(315deg,rgba(0,212,255,0.25),transparent_42%)]" />
      ...
      <div className="relative z-10 flex flex-1 flex-col min-h-0">
        <Nav current={page} onChange={goTo} />

        <div className="relative z-10 flex-1 min-h-0">
          <AnimatePresence mode="wait" custom={warp}>
            <motion.div
              key={page}
              custom={warp}
              ...
              animate="center"
              exit="exit"
              transition={{ duration: 0.45, ease: easeWarp }}
              className="flex h-full"
            >
              <PageComponent onChange={goTo} />
            </motion.div>
          </AnimatePresence>
        </div>
      </div>

      <PageIndicator current={page} total={pages.length} />
